...
export interface MicrosoftAgentAgentLike {
  name?: string;
  role?: string;
...
/**
 * MicrosoftAgentAdapter -- wraps MicrosoftAgent agents with Agentomy governance.
 * Does not import the MicrosoftAgent framework. The consumer provides their agent.
 */
export class MicrosoftAgentAdapter extends BaseAdapter {
  constructor(config?: AgentomyConfig) {
    super(config || {}, 'microsoftagent_agent');
  }

  wrap(agent: MicrosoftAgentAgentLike): GovernedAgent {
    const agentName = agent.name || agent.role || 'microsoftagent_agent';
