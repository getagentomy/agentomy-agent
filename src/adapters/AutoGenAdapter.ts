...
export interface AutoGenAgentLike {
  name?: string;
  role?: string;
...
/**
 * AutoGenAdapter -- wraps AutoGen agents with Agentomy governance.
 * Does not import the AutoGen framework. The consumer provides their agent.
 */
export class AutoGenAdapter extends BaseAdapter {
  constructor(config?: AgentomyConfig) {
    super(config || {}, 'autogen_agent');
  }

  wrap(agent: AutoGenAgentLike): GovernedAgent {
    const agentName = agent.name || agent.role || 'autogen_agent';
